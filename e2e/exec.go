...
func startBastionTunnel(
	...
	for time.Now().Before(deadline) {
		...
		if err == nil {
			if _, loaded := localPortStore.LoadOrStore(localPort, "present"); !loaded {
				break
			}
		}
		select {
...
func sshClientConfig(user string, privateKey []byte) (*ssh.ClientConfig, error) {
	...
	return &ssh.ClientConfig{
		...
		HostKeyCallback: ssh.InsecureIgnoreHostKey(), // same as StrictHostKeyChecking=no
...
func runSSHCommand(
	...
	return runSSHCommandWithPrivateKeyFile(port, command, config.VMSSHPrivateKey)
}

func copyScriptToRemoteIfRequired(client *ssh.Client, command string) (string, error) {
	...
	remotePath := filepath.Join("/home/azureuser", filepath.Base(tmpFile.Name()))
...
func runSSHCommandWithPrivateKeyFile(
	...
	command, err = copyScriptToRemoteIfRequired(client, command)
	if err != nil {
		return nil, err
	}
